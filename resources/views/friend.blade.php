        ...
        @forelse($acceptedFriends as $friendship)
            <div class="col">
                <div class="card h-100">
                    <img src="{{ $friendship->sender_id == Auth::id() ? ($friendship->receiver->profile_picture ?: asset('assets/default-profile.jpg')) : ($friendship->sender->profile_picture ?: asset('assets/default-profile.jpg')) }}" class="card-img-top mx-auto" style="width: 8rem; height: 8rem;">
                    <div class="card-body">
                        <h5 class="mb-2">@lang('lang.name'): {{ $friendship->sender_id == Auth::id() ? $friendship->receiver->name : $friendship->sender->name }}</h5>
                        <p class="mb-2">@lang('lang.field_of_work'): {{ $friendship->sender_id == Auth::id() ? $friendship->receiver->field_of_work : $friendship->sender->field_of_work }}</p>
                        <p class="mb-2">@lang('lang.gender_value'): {{ $friendship->sender_id == Auth::id() ? $friendship->receiver->gender : $friendship->sender->gender }}</p>
